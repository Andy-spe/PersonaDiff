...
from models import PersonaDiff
from utils import *
import random
...
def trainer_diff(args, train_loader, dev_loader, model, diffusion, optimizer, epoch=100, device="cuda"):
    ...
    for e in range(epoch + 1):
        ...
        for i, (audio, vertice, template, one_hot, file_name) in pbar:
            ...
            if args.use_augmentation and random.random() > 0.7:
                mask_len = random.randint(1, vertice.shape[0]//10)
                start_idx = random.randint(0, vertice.shape[0]-mask_len)
                vertice[start_idx:start_idx+mask_len] = 0
            ...
            if args.use_augmentation and random.random() > 0.5:
                noise = torch.randn_like(vertice) * 0.02
                vertice += noise
        ...
        for audio, vertice, template, one_hot_all, file_name in dev_loader:
            ...
            val_subject = file_name[0].split("_")[0]
            if val_subject in train_subjects_list:
                condition_subject = val_subject
...
def main():
    ...
    parser.add_argument("--wav_path", type=str, default="", help='path of the audio signals')
    parser.add_argument("--vertices_path", type=str, default="", help='path of the ground truth')
    ...
    parser.add_argument("--result_path", type=str, default="result500", help='path to the predictions')
